...
function PDF (path, opts,execOptions) {
  if (!(this instanceof PDF)) return new PDF(path, opts,execOptions);
  ...
  this._execPath ='pdfinfo';
  if(process.platform=='win32' || process.platform=='win64'){
    if(process.arch=='x64'){
      this._execPath =pathModel.join(__dirname,'../common/xpdf/bin64','pdfinfo');
    }else{
      this._execPath =pathModel.join(__dirname,'../common/xpdf/bin32','pdfinfo');
    }
  }
...
PDF.prototype.exec = function(next) {
  ...
  var process = spawn(this._execPath, this._options.concat([this._input]));
...
PDF.prototype.getPageCount = function(onComplete){
  this.info((err, meta)=>{
